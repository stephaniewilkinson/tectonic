...
require_relative 'withings_activity'
...
class Tectonic < Roda
  ...
  module WithingsWorkouts
    ...
    # rather than "Calisthenics" -- see `WithingsActivity::CATEGORIES`. Naming a category and deciding whether it
    ...
    # `WithingsActivity::CATEGORIES` and this kept the decision. Had the table been added
    # beside this one with its own copies of 16 and 17, there would be two answers to "what is
    # 17 called" and, sooner or later, two answers to "does 17 count as lifting", which is the
    # question `CATEGORY_BONUS` is the whole weight of.
    ...
    # `WithingsPairing.offer` makes before it writes: one query and one spelling of "is there
    ...
    def fetched?(account_id, window)
      ...
      DB.transaction { found.each { |activity| WithingsActivity.store(account_id, activity) } }
      true
    end
    ...
    # not offering. There is at most one such row per session, because `WithingsPairing.offer`
    ...
    # and not the whole of `WithingsActivity::CATEGORIES` now that there is a whole of `CATEGORIES` to read.
    ...
    def because(row, window)
      said = WithingsActivity.called(row[:category]) if LIFTING.include?(row[:category])
