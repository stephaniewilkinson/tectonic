...
describe 'the title on a page you are signed in for' do
  ...
  it 'is an h1 at one size, on every page' do
    ['/', '/start', '/workouts', '/workouts/new', "/workouts/#{@workout}",
     ...
     "/exercises/#{@exercise}/edit", '/programs', "/programs/#{@program.id}", '/settings',
     '/connections', '/volume'].each { |path| assert_page_title(path) }
  end

  # The same field was called four things in four templates, and this form was the fifth:
  # the column name, rendered to whoever is logging a set.
  #
  # It reads as a question now rather than as the bare noun #129 proposed, and that is the
  # better answer arriving from somewhere else: #166 folded the two set forms into one
  # partial and #160 settled that a boolean answered by a tick is asked as a question --
  # which is why the workout record heads these same two columns "Warmup?" and
  # "Completed?". A label that matches the column it fills is worth more than a label that
  # matches the wording in the issue. What #129 was actually about is the last assertion:
  # no column name reaches a lifter.
